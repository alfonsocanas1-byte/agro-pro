...
class AgroProApp:
    ...
    def enviar_whatsapp(self, cliente, total):
        # Generar texto conciliado para WhatsApp
    ...
    def render_ui(self):
        ...
        with col1:
            ...
            # Sección Fotografía - Redirección lógica a fotografiasuniverso.py
            st.markdown("---")
            st.write("📷 **Fotografías del Universo**")
            if st.button("Ingresar a fotografiasuniverso.py"):
                st.info("Visualizando catálogo en módulo fotouniverso...")
                # Aquí se integraría la lógica de importación o navegación
            
            cant_f = st.number_input("Cantidad de Fotografías ($25,000)", min_value=0, step=1)
            ...
            # Sección ACOuniverso - Redirección lógica a acouniverso.py
            st.markdown("---")
            st.write("🎨 **Ilustraciones ACOuniverso**")
            if st.button("Ingresar a acouniverso.py"):
                st.info("Visualizando catálogo en módulo ACOuniverso...")

            cant_a = st.number_input("Cantidad de Ilustraciones ($25,000)", min_value=0, step=1)
